...
def test_doctor_status_label_escalates_by_severity():
    ...
    assert _doctor_status_label(warnings=[], pending_actions=[{"tool_name": "x"}], logs={"event_errors": 0, "trace_issues": 0, "loop_guards": 0}) == "Warning"
    assert _doctor_status_label(warnings=[], pending_actions=[], logs={"event_errors": 0, "trace_issues": 1, "loop_guards": 0}) == "Warning"
    assert _doctor_status_label(warnings=[], pending_actions=[], logs={"event_errors": 1, "trace_issues": 0, "loop_guards": 0}) == "Immediate check"
...
def test_memory_review_filter_hides_empty_sections(monkeypatch):
    ...
    assert "Corrected old memories" in result
    assert "Learned / kept new" not in result
    assert "Ignored as duplicates" not in result
