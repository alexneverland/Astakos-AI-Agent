...
# ─────────────────────────────────────────────────────────────────────────────
# 5. shift_logic — bounded weekly statements auto-apply; unbounded or ambiguous remain conservative
# ─────────────────────────────────────────────────────────────────────────────

class TestShiftWeek:
    FACT = "[USER_FACT] αυτη εβδομαδ δουλευω απογευμα"

    def test_weekly_shift_auto_applies(self):
        d = _first(self.FACT, "shift_logic")
        assert d["decision"] == "auto_apply"
        assert d["auto_apply"] is True

    def test_score_above_auto_threshold(self):
        d = _first(self.FACT, "shift_logic")
        assert d["score"] >= _AUTO_APPLY_THRESHOLD
    ...
    def test_conservative_flag_absent(self):
        d = _first(self.FACT, "shift_logic")
        assert "shift_logic_conservative" not in d["ambiguity_flags"]
...
class TestReconcileStats:
    ...
    def test_bounded_shift_logic_auto_applies(self, monkeypatch):
        import services.routine_reconciler as rr
        ...
        assert len(shift_applied) == 1, "bounded shift_logic should be in auto_apply list"
        assert shift_applied[0]["decision"] == "auto_apply"
