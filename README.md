...
| System Doctor | `system_doctor(days=N)` gives a read-only runtime health summary from logs, traces, pending approvals, Messenger drafts, session backlog, and routine confirmations. Default is today (`days=1`). Ask `/doctor` or "δες αν όλα πάνε καλά" without opening the debug dashboard. |
