...
def extract_json_from_text(raw_text: str) -> dict | list | None:
    """
    [Mastro-Shield]: Extracts JSON structures (dict or list) safely from LLM output,
    ignoring conversational filler text, markdown code blocks, and fixing trailing commas.
    """
    if not raw_text:
        return None
...
from core.nl_config import (
    ...
    UTILS_QTY_INTENTS, UTILS_SYSTEM_RESETS, MESSENGER_WORKFLOW_TOKENS, CB_PARTNER_GIFT_WORDS
)
...
def detect_prompt_injection(user_input) -> bool:
    ...
    if has_signal and len(text_to_check) > 20:
        try:
            ...
            check_prompt = load_prompt("security_classifier.md").format(
                text_to_check=text_to_check[:500]
            )
