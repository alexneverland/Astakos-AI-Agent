...
SCOPES = ['https://www.googleapis.com/auth/drive'] # Πλήρης πρόσβαση στο Drive

def authenticate_google_drive():
    # print("Φόρτωση credentials...") # Κρατάμε τα prints πιο λιτά για να μην γεμίζει το log
    creds = None
    # Χρησιμοποιούμε τις απόλυτες διαδρομές που υποδείχθηκαν
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # Εδώ θα γινόταν η ροή OAuth αν τρεχόταν σε πραγματικό περιβάλλον χρήστη
            ...
            return None
    # print("Credentials φορτώθηκαν επιτυχώς (ή απέτυχαν)...")
    return creds

def upload_folder_recursive(service, local_path, drive_parent_id, exclude_items):
    # --- [MASTRO-CONFIG]: Τύποι αρχείων που ΔΕΝ θέλουμε στο Drive ---
    exclude_exts = {'.pdf', '.xlsx', '.xls', '.jpg', '.jpeg', '.png', '.ogg', '.mp3', '.zip'}
    
    uploaded_items = []
    
    # Έλεγχος αν ο φάκελος υπάρχει τοπικά
    if not os.path.exists(local_path):
        return uploaded_items

    for item_name in os.listdir(local_path):
        # 1. Έλεγχος για ονόματα (φακέλους όπως 'venv' ή 'messenger_profile')
        ...
        if os.path.isfile(current_local_path):
            # Παίρνουμε την κατάληξη (π.χ. .pdf)
        ...
        # 3. Περίπτωση: Φάκελος (Αναδρομή)
            ...
            try:
                ...
                # Κάνουμε την αναδρομική κλήση για να μπούμε μέσα στον φάκελο
...
def daily_backup_to_drive():
    ...
    try:
        ...
        # Δημιουργία φακέλου με ημερομηνία στο Google Drive
        ...
        # Έλεγχος αν υπάρχει ήδη φάκελος με αυτό το όνομα
        ...
        # Λίστα φακέλων/αρχείων προς εξαίρεση (μόνο στην αρχική κλήση)
